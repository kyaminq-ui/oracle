...
public class ArenaGenerator : MonoBehaviour
{
    ...
    [Tooltip("Afficher des gizmos de debug (zones de spawn, obstacles) dans la Scene view.")]
    ...
    void Awake()
    {
        // Synchroniser les dimensions AVANT que GridManager n'initialise la grille
        if (arenaConfig != null && gridConfig != null)
        {
    ...
    /// Si les dimensions dans ArenaConfig ont changé, appelez d'abord GridManager.Instance.RegenerateGrid().
    /// </summary>
    ...
    void Step5_RenderTiles()
    {
        ...
        // RNG dédié au rendu (seed dérivée) pour que les sprites soient reproductibles
        // indépendamment des variations dans l'algo d'obstacle.
        ...
        for (int x = 0; x < w; x++)
        {
            for (int y = 0; y < h; y++)
            {
                ...
                tileGO.transform.position  = cell.WorldPosition;
