...
public class OracleSetupWizard : EditorWindow
{
    ...
    void DrawSectionUI()
    {
        ...
        GUILayout.Space(4f);
        DrawHelpBox("4.2.1 — Layout complet : barres HP + timer centré + bas (passif, PA/PM, Deck, Fin de tour). " +
    ...
    void SetupCombatUI()
    {
        ...
        if (timerUI == null)
        {
            var timerGO = CreateUIPanel(canvas.transform, "TimerUI",
                ...
                new Vector2(-40f, -50f), new Vector2(80f, 40f));
            ...
            fillImg.color      = new Color(0.20f, 0.80f, 0.20f);
            StretchFull(fillGO.GetComponent<RectTransform>());
            timerUI.fillImage = fillImg;

            timerUI.timeText = AddTMPToPanel(timerGO.transform, "TimeText",
                Vector2.zero, 16, FontStyles.Bold).GetComponent<TextMeshProUGUI>();
